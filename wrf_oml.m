...
hml = double(nc_varget('wrfout_1800_SSTHR_subset2.nc','HML'));
hml_no = double(nc_varget('wrfout_1800_SST_NOANM_subset2.nc','HML'));
...
pcolorm(lat,lon,squeeze(hml(33,:,:))); shading interp; colormap(turbo); 
...
axesm('miller','MeridianLabel','on','MLabelParallel','south','MLabelLocation',...
    [-94:4:-74],'ParallelLabel','on','PLabelLocation',[12:4:26],'FontSize',12);
pcolorm(lat,lon,squeeze(hml_no(33,:,:))); shading interp; colormap(turbo); 
cb = colorbar;
cb.FontWeight = 'bold';
cb.FontSize = 16;
cb.Box = 'on';
